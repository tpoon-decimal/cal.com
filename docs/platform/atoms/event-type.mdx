...
One important setting in this tab is **Allow rescheduling past events**. When enabled, attendees can reschedule bookings even after the originally scheduled time has passed (for example, missed meetings). By default, this setting is **disabled**, which means reschedule links for past bookings will not work and attendees will be unable to reschedule them.

<Note>
If your attendees are reporting that their reschedule links have stopped working for past or missed bookings, check that the **Allow rescheduling past events** toggle is enabled in the **Advanced** tab of your event type settings.
</Note>

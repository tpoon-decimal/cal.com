...
One notable option is **Allow rescheduling past events**. By default, this setting is off, which means that when the original event time has already passed, the reschedule link redirects the attendee to a new booking page with their name and email pre-filled rather than performing a true reschedule. When you enable this setting, attendees can use the reschedule link to reschedule past bookings normally — this is useful when attendees missed a meeting and need to rebook for a new time while keeping the reschedule flow intact.

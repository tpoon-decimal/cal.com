...
The advanced tab includes a setting called **Allow rescheduling past events** that controls how reschedule links behave for bookings whose scheduled time has already passed.

- **When disabled (default):** If a booking's scheduled time has passed, clicking the reschedule link in the confirmation email will redirect the attendee to the booking details page instead of showing the rescheduling form. This prevents rescheduling of events that have already occurred.
- **When enabled:** Attendees can use reschedule links to book a new time even after the original meeting time has passed. This is useful for scenarios where a meeting was missed and the attendee needs to reschedule to a future time slot.

<Info>This setting is particularly helpful when attendees miss their appointments and need to use the reschedule link in their confirmation email to book a new time. Without this setting enabled, the reschedule link for a past booking will not allow selecting a new time.</Info>

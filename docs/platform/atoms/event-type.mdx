...
For team event types, you can enable a restriction schedule to further limit when bookings can occur. A restriction schedule is a separate availability schedule that acts as an additional filter on top of the host's regular availability — only time slots that fall within **both** the regular availability and the restriction schedule are bookable.

For example, if a host's regular availability is 9 AM–5 PM and the restriction schedule is set to 10 AM–4 PM, only the 10 AM–4 PM window is available for booking.

To enable a restriction schedule, toggle **Add Restriction Schedule** in the Availability tab of a team event type and select a schedule. When enabled, a **Use Booker Timezone** checkbox is available. By default, restriction schedule times are interpreted in the schedule's own timezone. When **Use Booker Timezone** is checked, the restriction times are applied relative to the booker's timezone instead.

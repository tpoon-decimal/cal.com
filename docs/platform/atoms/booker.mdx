...
<Note>
By default, rescheduling is only allowed for upcoming bookings. If you want attendees to be able to reschedule **past or missed bookings**, you must enable the **Allow rescheduling past events** setting in the **Advanced** tab of your [event type settings](/platform/atoms/event-type#event-type-settings). When this setting is disabled (the default), reschedule links for bookings whose scheduled time has already passed will not work.
</Note>
